...
- **Lifecycle**:
  - Created per user send, moved into a `QThread`, destroyed after signals emit `done`.
- **Used by**:
...
2. **Function name parsing & dispatch**  
   Currently: `_parse_function_call()` scans the raw model output for `call:<func_name>` among `VALID_FUNCTIONS`.  
