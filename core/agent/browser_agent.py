...
class BrowserAgent(QObject):
    ...
    def __init__(self, model_name="qwen3-vl:4b"):
        super().__init__()
        self.controller = BrowserController(headless=False)  # Headed — required for stealth and site compatibility
    ...
    def start_task(self, instruction: str):
        """
        Entry point called (via queued signal) from the GUI thread.

        Model-unloading steps are now wrapped in their own try/except so that
        a failure there (e.g. Ollama offline at that moment) does NOT silently
        crash the worker thread — it logs a warning and continues, letting the
        VLM agent attempt to load the model itself via Ollama's auto-load.
        """
        # --- Free VRAM before starting the VLM browser session ---
        try:
            unload_qwen("browser_agent_using_vlm")
        except Exception as e:
            print(f"[BrowserAgent] Warning: unload_qwen failed ({e}), continuing anyway.")

        try:
            ensure_exclusive_qwen(self.client.model_name)
        except Exception as e:
            print(f"[BrowserAgent] Warning: ensure_exclusive_qwen failed ({e}), continuing anyway.")
