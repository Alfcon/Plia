...
class PiperTTS:
    ...
    def _voice_url_from_name(self, voice_name: str) -> tuple[str, str]:
        """
        Build the HuggingFace download URLs for a Piper voice name.
        Pattern: en_GB-northern_english_male-medium
          → lang=en, lang_region=en_GB, voice=northern_english_male, quality=medium
        Returns (onnx_url, config_url).
        """
        base = "https://huggingface.co/rhasspy/piper-voices/resolve/main"
        parts = voice_name.split("-")
        lang_region = parts[0]                  # e.g. "en_GB"
        quality     = parts[-1]                 # e.g. "medium"
        voice       = "-".join(parts[1:-1])     # e.g. "northern_english_male"
    ...
    def _download_model_by_name(self, voice_name: str) -> str:
        """Download a voice model by name if not already present. Returns local .onnx path."""
        ...
        if not model_path.exists():
            ...
            print(f"{CYAN}[TTS] Downloading voice model ({voice_name})...{RESET}")
            try:
                r = http_session.get(onnx_url, stream=True, timeout=60)
                r.raise_for_status()
                with open(model_path, "wb") as f:
                    for chunk in r.iter_content(chunk_size=8192):
                        f.write(chunk)
                r2 = http_session.get(config_url, timeout=10)
                r2.raise_for_status()
                with open(config_path, "wb") as f:
                    f.write(r2.content)
                print(f"{GREEN}[TTS] ✓ Voice model downloaded: {voice_name}{RESET}")
            except Exception as e:
                print(f"{YELLOW}[TTS] Failed to download voice '{voice_name}': {e}{RESET}")
                # Fall back to existing model if download fails
                if not model_path.exists():
                    return None
    ...
    def change_voice(self, voice_name: str) -> bool:
        """
        Switch to a different Piper voice at runtime.
        ...
        new_path = self._download_model_by_name(voice_name)
        if not new_path:
            print(f"{YELLOW}[TTS] Could not obtain model for '{voice_name}' — keeping current voice.{RESET}")
            return False
    ...
    def _download_model(self):
        """Download voice model if not present. Uses the current VOICE_MODEL setting."""
        result = self._download_model_by_name(self.VOICE_MODEL)
        if result:
            return result
        # Last-resort fallback: try the original hardcoded model
        fallback = "en_GB-northern_english_male-medium"
        if self.VOICE_MODEL != fallback:
            print(f"{YELLOW}[TTS] Falling back to default voice: {fallback}{RESET}")
            self.VOICE_MODEL = fallback
            return self._download_model_by_name(fallback)
        return None
    ...
    def initialize(self):
        ...
        try:
            ...
            # Read voice from settings (fall back to class default)
            try:
                from core.settings_store import settings as _s
                saved_voice = _s.get("tts.voice", self.VOICE_MODEL)
                if saved_voice:
                    self.VOICE_MODEL = saved_voice
            except Exception:
                pass  # settings not available yet — use default
