...
| Continuous monitoring agent (long-running watchers) | 🟡 Partially (model persistence timeout, but no watchers) | There is model “unload after inactivity” logic (`core/model_persistence.py` search hits). No generic “watchers” agent loop found in explored files. |
