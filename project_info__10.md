...
- **Morning digest scheduled agent (open-jarvis).**  
  - **Added** ✅  
  - Evidence: `gui/app.py` contains morning digest timer loop and `core/settings_store.py` defines `morning_digest`.
...
- **Continuous monitoring agent (open-jarvis) — long-running background watchers.**  
  ...
  - No dedicated “watcher” framework found in the code we inspected.
...
- **Gender-switchable voice persona (Jarvis male / Friday female) (GauravSingh).** ❌
...
| `core/function_executor.py` | Executes routed actions + returns structured results |
